...
def cost_jmucc(Quket, print_level, theta_lists):
    ...
    for istate in range(nstates):
        ...
        occ_list_tmp = int2occ(Quket.multi.init_states_info[istate][0][1])
    ...
    t0_ = time.time()
    for istate in range(nstates):
        det = Quket.multi.init_states_info[istate][0][1]
        cf.ncnot = 0
...
def deriv_jmucc(Quket, theta_lists):
    ...
    Hstates = []
    for istate in range(nstates):
        Hstate = evolve(Quket.operators.jw_Hamiltonian, states[istate], parallel=True)
