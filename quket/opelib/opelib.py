...
def OpenFermionOperator2QulacsObservable(operator, n_qubits):
    """
...
def OpenFermionOperator2QulacsGeneralOperator(operator, n_qubits):
    """Function
...
def single_operator_gradient(a, i, state, jw_Hamiltonian, numerical=True, debug=False):
    """Function
    ...
    Args:
        ...
        jw_Hamiltonian (QubitOperator): Jordan_Wigner transformed Hamiltonian
    """
    # Analytical
    n_qubits = state.get_qubit_count()
    fermi = FermionOperator(f"{a}^ {i}", 1.) + FermionOperator(f"{i}^ {a}", -1.)
    jw_fermi = jordan_wigner(fermi)
    Hfermi = commutator(jw_Hamiltonian, jw_fermi)
    ...
    if debug:
        ham = create_observable_from_openfermion_text(str(jw_Hamiltonian))
...
def double_operator_gradient(b, a, j, i, state, jw_Hamiltonian, debug=False):
    """Function
...
def spin_double_grad(b, a, j, i, state, jw_Hamiltonian , Quket, bs_orbitals=None, numerical=True, H_state=None):
    """Function
...
#                raise ValueError('<(H-E)P> has to be zero')
            ...
            Ak_fermi = FermionOperator( str( b ) +'^ '+ str( a ) +'^ '+ str( j ) +' '+ str( i ) , 1.0)\
                      +FermionOperator( str( i ) +'^ '+ str( j ) +'^ '+ str( a ) +' '+ str( b ) ,-1.0)
            Ak_jw = jordan_wigner( Ak_fermi ) 
            AkH_jw = Ak_jw * (jw_Hamiltonian -  E0) 
            AkH_observable = OpenFermionOperator2QulacsGeneralOperator(AkH_jw, n_qubits)
...
def spin_single_grad( a , i , state , jw_Hamiltonian, Quket, bs_orbitals=None, numerical=True, H_state=None):
    """Function
    Compute derivative d < e(-theta A) HP exp(theta A) > / d theta
...
def pauli_grad(pauli, state , jw_Hamiltonian, Quket, bs_orbitals=None, numerical=True, H_state=None):
    """Function
    Compute derivative d < e(-theta A) HP exp(theta A) > / d theta
    where A is a single excitation operator. For spin-projection, P is taken into account.
    For non-spin-projection, P is set to 1.
    ...
    if numerical:
        ...
        if Quket.projection.SpinProj:
            ...
            HstateP_p    = evolve(jw_Hamiltonian, state_p)
            HstateP_m    = evolve(jw_Hamiltonian, state_m)
