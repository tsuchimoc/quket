...
from quket.tapering import tapering_off_operator, transform_pauli_list
...
def get_pauli_list_hamiltonian(Quket, anti=False, threshold=0):
    """
...
#def get_pauli_list_uccsd_sf(Quket, singles=True):
def get_pauli_list_uccsd_sf(Quket, no, nv, DS=0, x=0, singles=True, doubles=True):
    """
...
def create_pauli_list_gs(norbs, pauli_list, ncnot_list=None, disassemble_pauli=False):
    ####################
    ##    Singles     ##
    ####################
...
def create_pauli_list_gd(norbs, pauli_list, ncnot_list=None, disassemble_pauli=False):
    ####################
    ##    Doubles     ##
    ####################
...
def create_pauli_list_gt(norbs):
    pauli_list = []
    ### AAA
...
def create_pauli_list_gq(norbs):
    pauli_list = []
    ### AAAA
...
def get_pauli_list_uccgsd(norbs, DS=0, singles=True, disassemble_pauli=False):
    """Function:
    ...
    for rank in order:  
        if rank == 'singles':
            pauli_list, ncnot_list = create_pauli_list_gs(norbs, pauli_list, ncnot_list=ncnot_list, disassemble_pauli=disassemble_pauli)
        if rank == 'doubles':    
            pauli_list, ncnot_list = create_pauli_list_gd(norbs, pauli_list, ncnot_list=ncnot_list, disassemble_pauli=disassemble_pauli)
    return pauli_list, ncnot_list

def get_pauli_list_uccgsdt(norbs):
    """Function:
    ...
    for rank in order:  
        if rank == 'singles':
            pauli_list, ncnot_list = create_pauli_list_gs(norbs, pauli_list)
        if rank == 'doubles':    
            pauli_list, ncnot_list = create_pauli_list_gd(norbs, pauli_list)
        if rank == 'triples':    
            pauli_list += create_pauli_list_gt(norbs)
    pauli_list = get_unique_list(pauli_list, sign=True)
    return pauli_list

def get_pauli_list_uccgsdtq(norbs, disassemble_pauli=False):
    """Function:
    Create Pauli list for UCCGSDT.
    ...
    for rank in order:  
        ...
        if rank == 'triples':    
            pauli_list += create_pauli_list_gt(norbs)
        if rank == 'quadruples':    
            pauli_list += create_pauli_list_gq(norbs)
    pauli_list = get_unique_list(pauli_list, sign=True)
    return pauli_list

def get_pauli_list_uccgsd_sf(norbs, DS=0, singles=True):
    """Function:
    ...
    if Quket.adapt.mode == "original":
        ...
        for pauli in pauli_list:
            ...
            if pauli_new != QubitOperator('',0):
                ...
                if commutator(Quket.operators.jw_Sz,pauli_new) != zero:
                    prints(commutator(Quket.operators.jw_Sz,pauli_new))
                    prints(type(commutator(Quket.operators.jw_Sz,pauli_new)))
                    prints(commutator(Quket.operators.jw_Sz,pauli_new)==zero)
                    error("Error!")
                if commutator(Quket.operators.jw_Number,pauli_new) != zero:
                    prints(f'original pauli = {pauli}')
                    prints('\n new pauli = ',pauli_new, ' \nN non-commutative?', commutator(Quket.operators.jw_Number,pauli_new))
                pauli_list_new.append(pauli_new)
            if not discard:
                if pauli_new1 != QubitOperator('',0):
                    if commutator(Quket.operators.jw_Sz,pauli_new1) != zero:
                        prints(commutator(Quket.operators.jw_Sz,pauli_new1))
                        error("Error!")
...
def get_allowed_pauli_list(Tapering, pauli_list):
    ...
    allowed_pauli_list = []
    from quket.tapering.tapering import transform_pauli
    i = 0
    for pauli in pauli_list:
        i+=1
